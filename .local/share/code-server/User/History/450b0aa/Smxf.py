...
if 'License Type' in df.columns and 'City' in df.columns and 'Original Issue Date' in df.columns:
    ...
    # Group by Year, License Type, and City
    ...
    # Multi-select filters for City and License Type
    selected_cities = st.multiselect(
        "Select Cities",
        options=license_city_time_series['City'].unique(),
        default=license_city_time_series['City'].unique()[:5]  # Default to first 5 cities
    )
    selected_license_types = st.multiselect(
        "Select License Types",
        options=license_city_time_series['License Type'].unique(),
        default=license_city_time_series['License Type'].unique()[:3]  # Default to first 3 license types
    )

...
if 'Business' in df.columns:
    ...
    business_activity_count = df['Business Name'].value_counts().reset_index()
    business_activity_count.columns = ['Business Name', 'Count']
    pie_chart = alt.Chart(business_activity_count).mark_arc().encode(
        theta=alt.Theta(field='Count', type='quantitative'),
        color=alt.Color(field='Business Name', type='nominal'),
        tooltip=['Business Name', 'Count']
    ).properties(width=600, height=400)

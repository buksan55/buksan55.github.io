...
if 'Original Issue Date' in df.columns and 'Business Name' in df.columns:
    ...
    # Group by Year and Business Name
    ...
    top_businesses = business_time_series.groupby('Business Name')['Count'].sum().nlargest(10).index
    filtered_data = business_time_series[business_time_series['Business Name'].isin(top_businesses)]

    # Line Chart with y-axis range set to 0.9 to 1.1
    line_chart = alt.Chart(filtered_data).mark_line().encode(
        x='Year:O',
        y=alt.Y('Count:Q', scale=alt.Scale(domain=[0.9, 1.1])),
    ...
    st.write("The dataset does not have the required columns 'Original Issue Date' or 'Business Name'.")

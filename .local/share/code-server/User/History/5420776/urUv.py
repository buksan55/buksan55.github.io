...
if 'Original Issue Date' in df.columns and 'Business Name' in df.columns and 'City' in df.columns:
    ...
    # Group by Year, Business Name, and City
    business_city_time_series = df.groupby([df['Original Issue Date'].dt.year, 'Business Name', 'City']).size().reset_index(name='Count')
    business_city_time_series.columns = ['Year', 'Business Name', 'City', 'Count']

    st.write("### Licenses Issued Over Time by Business Name and City")
    ...
    line_chart = alt.Chart(filtered_data).mark_line().encode(
        ...
        tooltip=['Year:O', 'Business Name:N', 'City:N', 'Count:Q'],
        strokeDash='City:N'  # Different line styles for different cities
